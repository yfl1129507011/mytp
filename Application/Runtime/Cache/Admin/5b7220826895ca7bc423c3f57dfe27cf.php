...
<div class="table-class">
	...
	<table id="box" class="center">
		...
		<?php if(!empty($_list)): if(is_array($_list)): foreach($_list as $k=>$value): ?><tr>
			<td><input class="ids" type="checkbox" name="ids[]" value="<?php echo ($value["mid"]); ?>" /></td>
			...
			<td align="center">
				<input type="button" class="button border-blue ajax-get" value="修改" query="<?php echo U('edit?mid='.$value['mid']);?>" />
				<input type="button" class="button border-red confirm ajax-get" value="删除" query="<?php echo U('del?id='.$value['mid']);?>" />
			</td>
		</tr><?php endforeach; endif; endif; ?>
	</table>
	<div class="page"><?php echo ($_show); ?></div>
</div>
<script>
	$(function(){
		//回车搜索
